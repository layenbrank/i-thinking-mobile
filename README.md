...
默认 `extra.apiBaseUrl = http://127.0.0.1:3000`。客户端会自动改写 loopback：

| 运行环境                    | 实际请求地址                 |
| --------------------------- | ---------------------------- |
| 真机（Expo Metro 在局域网） | `http://<电脑局域网IP>:3000` |
| Android 模拟器              | `http://10.0.2.2:3000`       |
| iOS 模拟器 / Web            | `http://127.0.0.1:3000`      |

| 键                                                 | 说明                                                 |
| -------------------------------------------------- | ---------------------------------------------------- |
| `EXPO_PUBLIC_API_BASE_URL` / `extra.apiBaseUrl`    | rust-service 根地址（显式局域网/线上地址不会被改写） |
| `EXPO_PUBLIC_DEFAULT_MODEL` / `extra.defaultModel` | gateway 模型名                                       |
| `EXPO_PUBLIC_TENANT_ID` / `extra.tenantId`         | 可选，写入 `X-Tenant-ID`                             |

需先启动 rust-service（监听 `0.0.0.0:3000`）以及 captcha 侧车：`docker compose up -d gocaptcha redis`。开发联调可临时关闭 `auth.captcha.enabled`（仍须提交 `captchaKey`/`captchaValue` 字段）。
